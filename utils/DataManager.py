...
class DataManager:
    ...
    # 发送数据底层函数，worker_id,switch_id,degree,offset偏移（从该位置读取并发送若单线程为则0）,
    # step步长（发送offset开始，长为step的数据，目前offset=0，step=数据总长）
    def _send_data(self, worker_id, switch_id, degree, offset, step, sequence=0, end=False):
        ...
        for i, index in enumerate(range(offset, offset + step, DATA_NUM)):
            left = index    # 0,32,64,96,...
            pkt_id = i      # 0, 1, 2, 3,...
            aggindex = mod((sequence + pkt_id), 16384) # NUM_REGISTER=16384，为交换机上寄存器数量
            ...
            nga = struct.pack( # ngaa包头部分
                ...
                worker_id,          # worker的编号
                degree,             # =0时表示聚合结束
                0,                  # overflow_isack_ecn_resend_time
                aggindex,           # sequence_id % 交换机总寄存器数量
                switch_id,          # 在哪个编号的交换机聚合
                sequence + pkt_id   # 交换机上根据这个来区分，来自不同client的包，这个一样就聚合
                                    # 由于未使用多线程，sequence=0，按pkt_id区分即可
            )
            for d in self.data[left:left + DATA_NUM]:
                nga += d
                count += 1
            s.sendto(nga, (self.dst_ip, 0)) #发送UDP数据。MAC和IP层无需定义，将数据发送到套接字（ipaddr，port），返回值是发送的字节数。
