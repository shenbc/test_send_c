...
# 和headers.p4的ipv4+ngaa包头一致
class NGATotal(Structure):
    _fields_ = [
        ("version_ihl", c_ubyte),                       # IPv4段
        ("tos", c_ubyte),
        ("len", c_ushort),
        ("id", c_ushort),
        ("offset", c_ushort),
        ...
        ("src", c_uint),
        ("dst", c_uint),                                # IPv4段结束
        ("worker_map", c_byte * 4),                     # worker id
        ("degree", c_ubyte),                            # =0时表示聚合结束
        ("overflow_isack_ecn_resend_time", c_ubyte),    # isack=1表示来自server.py
        ("index", c_byte * 4),                          # sequence_id % 交换机总寄存器数量
        ("switch_id", c_ubyte),                         # 在哪个编号的交换机聚合
        ("sequence_id", c_byte * 4),                    # 根据此判断是否聚合
        ("payload", c_int * DATA_NUM)
    ]
